...
- [最小覆盖子串](#最小覆盖子串)
  - [题目](#题目)
  - [最后3个用例超时](#最后3个用例超时)
  - [滑动窗口算法通用思想](#滑动窗口算法通用思想)
    - [一、最小覆盖子串](#一最小覆盖子串)
    - [二、找到字符串中所有字母异位词](#二找到字符串中所有字母异位词)
    - [三、无重复字符的最长子串](#三无重复字符的最长子串)
    - [最后总结](#最后总结)
...
## 滑动窗口算法通用思想
本文详解「**滑动窗口**」这种`高级双指针技巧的算法框架`，带你秒杀几道难度较大的子字符串匹配问题：
- 最小覆盖子串
- 找到字符串中所有字母异位词
- 无重复字符的最长子串
...
![hdck1.png](../resource/assets/算法/hdck1.png)

增加 right，直到窗口 [left, right] 包含了 T 中所有字符：

![hdck2.png](../resource/assets/算法/hdck2.png)

现在开始增加 left，缩小窗口 [left, right]。

![hdck3.png](../resource/assets/算法/hdck3.png)

直到窗口中的字符串不再符合要求，left 不再继续移动。

![hdck4.png](../resource/assets/算法/hdck4.png)
...
这个算法的时间复杂度是 O(M + N)，M 和 N 分别是字符串 S 和 T 的长度。因为我们先用 for 循环遍历了字符串 T 来初始化 needs，时间 O(N)，之后的两个 whilewhile 循环最多执行 2M 次，时间 O(M)。
